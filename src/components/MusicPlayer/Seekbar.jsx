...
const Seekbar = ({ value, min, max, onInput, setSeekTime, appTime, fullScreen, onSeekEnd }) => {
  // converts the time to format 0:00
  const getTime = (time) => `${Math.floor(time / 60)}:${(`0${Math.floor(time % 60)}`).slice(-2)}`;

  return (
    <div className={` ${!fullScreen ? 'hidden sm:flex':' flex mt-5'}   flex-row items-center`}>
      <button type="button" onClick={(e) =>{e.stopPropagation(); setSeekTime(appTime - 5); onSeekEnd && onSeekEnd(appTime - 5);}} className="hidden lg:mr-4 lg:block text-white">
      ...
      <input onClick={(event) => {event.stopPropagation();}}
        ...
        max={max}
        onInput={onInput}
        onMouseUp={(e) => onSeekEnd && onSeekEnd(e.target.value)}
        onTouchEnd={(e) => onSeekEnd && onSeekEnd(e.target.value)}
      ...
      <button type="button" onClick={(e) =>{e.stopPropagation(); setSeekTime(appTime + 5); onSeekEnd && onSeekEnd(appTime + 5);}} className="hidden lg:ml-4 lg:block text-white">
